...
// ---------------------------------------------------------------------------
// Status-driven configuration for the transfer confirmation email.
// Add/adjust entries here — the template below reads from this object,
// so you never have to touch the HTML to support a new status.
// ---------------------------------------------------------------------------
function getStatusConfig(status, failureReason) {
  const configs = {
    pending: {
      headerLabel: "Transaction Pending",
      heading: "Your Transfer Is On Its Way",
      bodyLine:
        "We're writing to confirm that your transfer has been initiated and is currently <strong>pending</strong>. A summary of the transaction is provided below for your records.",
      color: "#6b4423",
      gradientFrom: "#a97142",
      accent: "#f0d9c0",
    ...
    completed: {
      ...
      bodyLine:
        "We're writing to confirm that your transfer has been <strong>successfully completed</strong>. A summary of the transaction is provided below for your records.",
      color: "#1e7e34",
      gradientFrom: "#4caf68",
      accent: "#c9f0d3",
    ...
    failed: {
      headerLabel: "Transaction Failed",
      heading: "Your Transfer Could Not Be Completed",
      bodyLine: `We regret to inform you that your recent transfer <strong>could not be processed</strong> due to an <strong>${
        failureReason || "authorization failure"
      }</strong>. No funds have been debited from your account as a result of this failed attempt. Please review the transaction details below, and feel free to contact our support team if you require further assistance.`,
      color: "#b02a2a",
      gradientFrom: "#e05c5c",
      accent: "#f5c6c6",
      dateLabel: "Date Attempted",
      amountLabel: "Amount Attempted",
      showFee: false, // failed transfers show fee as $0.00 (not charged)
  ...
  // but keeps the real status word so nothing is silently hidden.
  return (
    configs[status] || {
      ...configs.pending,
      headerLabel: status || "Transaction Update",
    }
  );
}
...
const sendTransferConfirmationEmail = async ({
  ...
  firstName,
  transaction,
  timeZone, // may be undefined if not sent from frontend
  failureReason, // optional — pass this in when status is "failed"
}) => {
  if (process.env.NODE_ENV !== "test") {
    try {
      const {
      ...
      const cfg = getStatusConfig(transaction.status, failureReason);

      const displayFee = cfg.showFee ? Number(transferFee).toFixed(2) : "0.00";
      const displayTotal = cfg.showFee
      ...
      const { data, error } = await resend.emails.send({
        from: process.env.RESEND_FROM || "Trenthe Bank <noreply@trenthe.com>",
        to: email,
        subject: `${cfg.headerLabel} - ${transaction.reference}`,
        html: `
    <div style="font-family: Arial, Helvetica, sans-serif; max-width: 600px; margin: 0 auto; background-color: #eef1f5; padding: 32px 20px;">
        <div style="background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 8px rgba(11,37,69,0.08); border: 1px solid #e2e6ec;">
            ...
            <!-- Header -->
            <div style="background-color: ${cfg.color}; padding: 32px 36px; text-align: center;">
                <span style="color: #ffffff; font-size: 15px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">${cfg.headerLabel}</span>
            </div>

            <div style="height: 3px; background: linear-gradient(90deg, ${cfg.gradientFrom}, ${cfg.color});"></div>
            ...
            <div style="padding: 36px;">
                ...
                <h2 style="color: ${cfg.color}; font-size: 22px; margin: 0 0 18px 0;">${cfg.heading}</h2>

                <p style="color: #444; font-size: 14px; line-height: 1.7; margin: 0 0 6px 0;">Dear ${recipientName || firstName || ""},</p>
                ...
                <div style="border: 1px solid #e2e6ec; border-radius: 8px; overflow: hidden;">
                    <div style="background-color: #f7f9fb; padding: 14px 20px; border-bottom: 1px solid #e2e6ec;">
                        <p style="margin: 0; color: ${cfg.color}; font-size: 13px; font-weight: bold;">Transaction Summary</p>
                    </div>
                    <table style="width: 100%; border-collapse: collapse;">
                        <tr>
                            <td style="padding: 13px 20px; color: #6b7280; font-size: 13px; border-bottom: 1px solid #eef1f5;">Reference</td>
                            <td style="padding: 13px 20px; text-align: right; color: ${cfg.color}; font-size: 13px; font-weight: 600; font-family: 'Courier New', monospace; border-bottom: 1px solid #eef1f5;">${transaction.reference}</td>
                        </tr>
                        <tr>
                            <td style="padding: 13px 20px; color: #6b7280; font-size: 13px; border-bottom: 1px solid #eef1f5;">${cfg.dateLabel}</td>
                        ...
                        <tr>
                            <td style="padding: 13px 20px; color: #6b7280; font-size: 13px; border-bottom: 1px solid #eef1f5;">${cfg.amountLabel}</td>
                        ...
                        ${
                          cfg.showFailureReason
                            ? `<tr>
                            <td style="padding: 13px 20px; color: #6b7280; font-size: 13px;">Failure Reason</td>
                            <td style="padding: 13px 20px; text-align: right; color: ${cfg.color}; font-size: 13px; font-weight: 600;">${cfg.failureReasonDisplay}</td>
                        </tr>`
                            : ""
                        }
                    </table>
                    <div style="background-color: ${cfg.color}; padding: 16px 20px;">
                        <table style="width: 100%;">
                            <tr>
                                <td style="color: #ffffff; font-size: 14px; font-weight: bold;">${cfg.totalLabel}</td>
                                <td style="text-align: right; color: ${cfg.accent}; font-size: 18px; font-weight: bold;">$${displayTotal}</td>
                            </tr>
                        </table>
                    </div>
                </div>
                ...
                <table style="width: 100%; margin-top: 28px;">
                    <tr>
                        <td style="width: 50%; padding-right: 8px;">
                            <a href="${transactionUrl}" style="display: block; text-align: center; background-color: ${cfg.color}; color: #ffffff; font-size: 13px; font-weight: bold; padding: 12px; border-radius: 6px; text-decoration: none;">View Transaction</a>
                        </td>
                        <td style="width: 50%; padding-left: 8px;">
                            <a href="${receiptUrl}" style="display: block; text-align: center; background-color: #ffffff; color: ${cfg.color}; font-size: 13px; font-weight: bold; padding: 11px; border-radius: 6px; text-decoration: none; border: 1px solid ${cfg.color};">Download Receipt</a>
                        </td>
                    </tr>
                </table>
                ...
                <p style="color: #444; font-size: 14px; line-height: 1.7; margin-top: 30px; margin-bottom: 0;">
                    ...
                    <strong style="color: ${cfg.color};">The Trenthe Bank Team</strong>
                </p>
            </div>

            <!-- Footer -->
            <div style="background-color: #f7f9fb; padding: 22px 36px; border-top: 1px solid #e2e6ec; text-align: center;">
                <p style="margin: 0 0 6px 0; color: ${cfg.color}; font-size: 12px; font-weight: bold; letter-spacing: 0.4px;">TRENTHE BANK</p>
